...
<body>
      ...
      <div class="row">
        ...
        <div class="col-md-4">
          ...
          <select class="form-select" name="regiao_id" id="regiao_id" required>
            ...
            $regiao = mysqli_query($conexao, "SELECT * FROM regiao ORDER BY nome");
            while ($reg = mysqli_fetch_assoc($regiao)) {
        ...
        <div class="col-md-4 mt-4">
          ...
          <select class="form-select" name="area_id" required>
            <option> Selecione </option>
            <?php
            $area = mysqli_query($conexao, "SELECT * FROM area ORDER BY nome");
        ...
        <div class="col-md-4 mt-4">
          ...
          name="dt_compra" value="<?=date('Y-m-d')?>">
        </div>
  ...
  <script>
    //se tiver alteraçao no campo regiao, dispara essa funçao
    $('#regiao_id').on('change', function () {
      //variavel que guarda id da regiao selecionada
      var regiaoId = $(this).val();
      //vamos chaar o arquivo php que vai carregar as cidades de acordo com regiao
      $.post('./backend/venda/buscar_cidade.php', { regiao_id: regiaoId },
        //funçao que vai retornar as cidades de acordo com a regiao
